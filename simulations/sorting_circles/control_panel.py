...
# Add the base directory to sys.path so shared module can be imported
...
class SortingCirclesControlPanel:
    ...
    def _generate_video_thread(self):
        """Generate video in background thread using VideoGenerator."""
        try:
            # Import VideoGenerator from shared module
            ...
            output_name = f"sorting_circles_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            fps = int(self.fps_var.get())
            quality = "high"
            
            # Generate video directly - returns Path object
